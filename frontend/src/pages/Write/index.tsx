...
const Write = () => {
  ...
  const handleImageState: ChangeEventHandler<HTMLInputElement> = (event: any) => {
    ...
    if (Number(imageList.length) + Number(images.length) > 10) {
        ...
        <div className='write-page'>
            ...
            <div className="write-body">
                ...
                <div className="button-bar-wrapper">
                  ...
                  <input type="file" multiple className="image-input" accept='image/*' ref={inputRef} onChange={handleImageState}/>
                  <button className='thumbnail-preview' onClick={handleThumbnailModal} disabled={isDisabledButton()}>썸네일 미리보기</button>
                </div>
                <textarea className='text-area' placeholder='글 내용을 입력해주세요'/>
              <button className="write-button" disabled={isDisabledButton()} onClick={handleUploadPosting}>게시물 업로드 하기</button>
              <Toast />
            </div>
        </div>
  )
